...
public class MyCourseList implements CourseCommand {
	...
	public void execute(Model model) throws Exception {
		...
		/*
		 * if (request.getParameter("weekDate") != null) { if
		 * (request.getParameter("direction").equals("prev")) { today =
		 * LocalDate.parse(request.getParameter("weekDate")).minusDays(1); }
		 * 
		 * else if (request.getParameter("direction").equals("next")) { today =
		 * LocalDate.parse(request.getParameter("weekDate")).plusDays(1); }
		 * 
		 * else if (request.getParameter("direction").equals("thisPage")) { today =
		 * LocalDate.parse(request.getParameter("weekDate")); } }
		 * 
		 * LocalDate monday =
		 * today.with(TemporalAdjusters.previousOrSame(DayOfWeek.MONDAY)); LocalDate
...
//		DTO_Course dto =new DTO_Course();
		...
		myCourseList=dao.getUserTotalItem(today, user_idn);
