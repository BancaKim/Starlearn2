...
<script>
    ...
    // Sort data by date for each level
    for (var level in level3Groups) {
        var combined = [];
        for (var i = 0; i < level3Groups[level].yymm.length; i++) {
            combined.push({
                yymm: level3Groups[level].yymm[i],
                aveScores: level3Groups[level].aveScores[i],
                passPercents: level3Groups[level].passPercents[i],
                sumApply: level3Groups[level].sumApply[i]
            });
        }
        combined.sort(function(a, b) {
            return sortByDate(a.yymm, b.yymm);
        });
        level3Groups[level].yymm = combined.map(item => item.yymm);
        level3Groups[level].aveScores = combined.map(item => item.aveScores);
        level3Groups[level].passPercents = combined.map(item => item.passPercents);
        level3Groups[level].sumApply = combined.map(item => item.sumApply);
    }
